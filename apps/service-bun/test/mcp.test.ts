...
describe("mcp toolkit", () => {
  it.effect("registers suggest-address tool metadata", () =>
    Effect.gen(function* () {
      const tools = yield* Effect.scoped(
        Effect.gen(function* () {
      ...
      expect(suggestTool?.description).toBe(
        "Suggest addresses using configured providers and strategies."
      );
  ...
  it.effect("handles suggest-address tool calls", () =>
    Effect.gen(function* () {
